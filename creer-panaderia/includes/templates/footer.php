    <footer class="row-footer">
      <div class="container">
        <div class="row">
          <div class="col-xs-5 col-xs-offset-1 col-sm-2 col-sm-offset-1 site-footer">
            <h5>Enlaces</h5>
            <ul class="list-unstyled">
              <li><a href="index.html">Inicio</a></li>
              <li><a href="contactus.html">Contacto</a></li>
            </ul>
          </div>
          <div class="col-xs-6 col-sm-5 site-footer contact-class">
            <h5>Contacto</h5>
            <h6>Nuestra direcci&oacute;n</h6>
            <address>
              Urquiza 1900<br>
              Quilmes, Buenos Aires<br>
              ARGENTINA<br>
	            <i class="fa fa-phone"></i>: 1151093079<br>
              <i class="fa fa-envelope"></i>: <a href="mailto:pedidospanaderiajl@gmail.com">pedidospanaderiajl@gmail.com</a>
            </address>
          </div>
          <div class="col-xs-12 col-sm-4 site-footer">
            <h5>Seguinos</h5>
            <div class="nav navbar-nav" style="padding: 10px 10px;">
              <a class="btn btn-social-icon btn-google-plus" href="http://google.com/+" target="_blank"><i class="fa fa-google-plus"></i></a>
              <a class="btn btn-social-icon btn-facebook" href="https://www.facebook.com/creerpanaderiapasteleria/" target="_blank"><i class="fa fa-facebook"></i></a>
              <a class="btn btn-social-icon btn-linkedin" href="http://www.linkedin.com/in/" target="_blank"><i class="fa fa-linkedin"></i></a>
              <a class="btn btn-social-icon btn-twitter" href="http://twitter.com/" target="_blank"><i class="fa fa-twitter"></i></a>
              <a class="btn btn-social-icon btn-youtube" href="http://youtube.com/" target="_blank"><i class="fa fa-youtube"></i></a>
              <a class="btn btn-social-icon" href="mailto:" target="_blank"><i class="fa fa-envelope-o"></i></a>
            </div>
          </div>
          <div class="col-xs-12 site-footer">
            <p style="padding:10px;"></p>
            <p align="center">&copy; Copyright 2018 Cr&eacute;er Panader&iacute;a - Desarrollado por <a class="develop_by" href="https://devsolutionsba.com/es/inicio">DevSolutionsBA</a></p>
          </div>
        </div>
      </div>
    </footer>
    <script src="bower_components/jquery/dist/jquery.min.js"></script>
    <script src="bower_components/bootstrap/dist/js/bootstrap.min.js"></script>
    <script src="js/main.js"></script>
  </body>
</html>
